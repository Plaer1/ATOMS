...
export default function rootMenuDropdown() {
   waitForElement('._1Ky59qmywxOUtNcI1cgmkX', rootMenuDropdownMain);
}

function rootMenuDropdownMain(){
...
wrapperDiv.setAttribute("class", "rootMenuWrapperDiv");
wrapperDiv.style.backgroundColor = 'rgba(var(--backgroundLightish))'; // Setting the background color
wrapperDiv.style.minWidth = '30px';
...
wrapperDiv.appendChild(buttonSVG); // Append buttonSVG to wrapperDiv
...
wrapperDiv.appendChild(menu);
    ...
    target.insertBefore(wrapperDiv, target.firstChild); // inserting the menu before the first child of the target
    
    }else{ console.log("fuckfuckfuckfuckfuckfuck");}


    // Window blur event, hides thing when window loses focus:
    //window.addEventListener("blur", function(event) {
        //const openPopup = document.querySelector(".DesktopUI.ContextMenuPopupBody");
        //if (!openPopup) {
			//menu.style.display = "none";
           // logoSVG.querySelector("#logoPath").setAttribute("fill", "#FFF");
